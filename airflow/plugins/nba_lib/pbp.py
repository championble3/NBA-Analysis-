...
def format_df(df1):
    ...
    for index, row in df1.iterrows():
        d = {'QUARTER': float('nan'), 'TIME_REMAINING': float('nan'), f'HOME_ACTION': float('nan'), f'AWAY_ACTION': float('nan'), f'HOME_SCORE': float('nan'), f'AWAY_SCORE': float('nan')}
        if row['Time']=='2nd Q':
            q = 2
        elif row['Time']=='3rd Q':
            q = 3
        ...
        try:
            ...
            d[f'HOME_SCORE'] = int(scores[0])
            d[f'AWAY_SCORE'] = int(scores[1])
            d[f'HOME_ACTION'] = row[list(df1.columns)[1]]
            d[f'AWAY_ACTION'] = row[list(df1.columns)[5]]
            if df is None:
